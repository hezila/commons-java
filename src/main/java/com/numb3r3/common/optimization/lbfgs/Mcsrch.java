...
public class Mcsrch {
    ...
    private static double dg = 0, dgm = 0, dginit = 0, dgtest = 0,
            dgx[] = new double[1], dgxm[] = new double[1], dgy[] = new double[1],
            dgym[] = new double[1], finit = 0, ftest1 = 0, fm = 0,
            fx[] = new double[1], fxm[] = new double[1], fy[] = new double[1],
            fym[] = new double[1];
     ...
     * which in turn is a slight modification of the subroutine <code>CSRCH</code>
     * of More' and Thuente. The changes are to allow reverse communication, and
     * do not affect the performance of the routine. This function, in turn, calls
     * <code>mcstep</code>.
     * <p/>
     ...
     * the function is bounded below, then there is always a step which satisfies
     * both conditions. If no step can be found which satisfies both conditions,
     * then the algorithm usually stops when rounding errors prevent further
     * progress. In this case <code>stp</code> only satisfies the sufficient
     * decrease condition.
     * <p/>
     *
     * @param n      The number of variables.
     * @param x      On entry this contains the base point for the line search. On exit
     ...
     * @param g      On entry this contains the gradient of the objective function at
     *               <code>x</code>. On exit it contains the gradient at
     *               <code>x + stp*s</code>.
     * @param s      The search direction.
     * @param stp    On entry this contains an initial estimate of a satifactory step
     ...
     * @param maxfev Termination occurs when the number of evaluations of the objective
     *               function is at least <code>maxfev</code> by the end of an
     *               iteration.
     * @param info   This is an output variable, which can have these values:
     *               <ul>
     *               <li><code>info = 0</code> Improper input parameters. <li> <code>
     *               info = -1</code> A return is made to compute the function and
     *               gradient. <li><code>info = 1</code> The sufficient decrease
     *               condition and the directional derivative condition hold. <li>
     *               <code>info = 2</code> Relative width of the interval of
     *               uncertainty is at most <code>xtol</code>. <li> <code>info = 3
     *               </code> Number of function evaluations has reached <code>maxfev
     *               </code>. <li><code>info = 4</code> The step is at the lower bound
     *               <code>stpmin</code>. <li><code>info = 5</code> The step is at the
     *               upper bound <code>stpmax</code>. <li><code>info = 6</code>
     ...
     * part of the Minpack project, June 1983, Argonne National
     * Laboratory. Java translation by Robert Dodier, August 1997.
     */

    public static void mcsrch(final int n, double[] x, double f, double[] g,
                              double[] s, int is0, double[] stp, double ftol, double xtol, int maxfev,
                              int[] info, int[] nfev, double[] wa, int[] iprint) {
        ...
        if (info[0] != -1) {
            ...
            if (iprint[0] > 0)
                System.err.println("new line search f=" + finit + " dg=" + dginit
                        + " stp=" + stp[0]);
        }
        ...
        while (true) {
            if (info[0] != -1) {
                ...
                if ((brackt[0] && (stp[0] <= stmin || stp[0] >= stmax || stmax - stmin <= xtol
                        * stmax))
            ...
            if (iprint[0] > 0)
                System.err.println(" msrch internal f=" + f + " dg=" + dg + " stx="
                        + stx[0] + " sty=" + sty[0] + " brackt=" + brackt[0] + " stp="
                        + stp[0]);
     ...
     * direction of the step. If <code>brackt[0]</code> is <code>true</code> when
     * <code>mcstep</code> returns then a minimizer has been bracketed in an
     * interval of uncertainty with endpoints <code>stx</code> and
     * <code>sty</code>.
     * <p/>
     * Variables that must be modified by <code>mcstep</code> are implemented as
     * 1-element arrays.
     *
     * @param stx1    Step at the best step obtained so far. This variable is modified
     *                by <code>mcstep</code>.
     * @param fx1     Function value at the best step obtained so far. This variable is
     *                modified by <code>mcstep</code>.
     * @param dx      Derivative at the best step obtained so far. The derivative must
     ...
     * @param sty1    Step at the other endpoint of the interval of uncertainty. This
     *                variable is modified by <code>mcstep</code>.
     * @param fy1     Function value at the other endpoint of the interval of
     *                uncertainty. This variable is modified by <code>mcstep</code>.
     * @param dy      Derivative at the other endpoint of the interval of uncertainty.
     *                This variable is modified by <code>mcstep</code>.
     * @param stp     Step at the current step. If <code>brackt</code> is set then on
     ...
     * @param brackt1 Tells whether a minimizer has been bracketed. If the minimizer has
     ...
     * @param info    On return from <code>mcstep</code>, this is set as follows: If
     ...
     * @author Jorge J. More, David J. Thuente: original Fortran version, as part
     * of Minpack project. Argonne Nat'l Laboratory, June 1983. Robert
    ...
    public static void mcstep(double[] stx1, double[] fx1, double[] dx,
                              ...
                              double dp, boolean[] brackt1, double stpmin, double stpmax, int[] info,
                              int[] iprint) {
        boolean bound;
        ...
        if ((brackt1[0] && (stp[0] <= Math.min(stx1[0], sty1[0]) || stp[0] >= Math
                .max(stx1[0], sty1[0])))
                || dx[0] * (stp[0] - stx1[0]) >= 0.0
                || stpmax < stpmin) {
            if (iprint[0] > 0)
                System.err.println("mcstep=0 " + brackt1[0] + " " + stp[0] + " "
                        + stx1[0] + " " + sty1[0] + " " + dx[0] + " " + stpmax + " "
                        + stpmin);
            return;
        }
        ...
        if (fp > fx1[0]) {
            ...
            s = max3(Math.abs(theta), Math.abs(dx[0]), Math.abs(dp));
            gamma = s
                    * Math.sqrt(Math.max(0, sqr(theta / s) - (dx[0] / s) * (dp / s)));
            if (stp[0] > stx1[0])
                gamma = -gamma;
            p = (gamma - dp) + theta;
