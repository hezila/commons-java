 ...
 * recent line search. See <tt>solution_cache</tt> below. Following is a message
 * from Jorge Nocedal:
 * <p/>
 * <pre>
...
public class LBFGS {
     ...
     * function and gradient evaluations are inexpensive with respect to the cost
     * of the iteration (which is sometimes the case when solving very large
     * problems) it may be advantageous to set <code>gtol</code> to a small value.
     * A typical small value is 0.1. Restriction: <code>gtol</code> should be
     * greater than 1e-4.
     */

    public static double gtol = 0.01;
     ...
     * 45 (1989) 503-528. The user is required to calculate the function value
     * <code>f</code> and its gradient <code>g</code>. In order to allow the user
     * complete control over these computations, reverse communication is used.
     * The routine must be called repeatedly under the control of the parameter
     * <code>iflag</code>. The steplength is determined at each iteration by means
     * of the line search routine <code>mcsrch</code>, which is a slight
     * modification of the routine <code>CSRCH</code> written by More' and
     * Thuente. The only variables that are machine-dependent are
     * <code>xtol</code>, <code>stpmin</code> and <code>stpmax</code>. Progress
     * messages and non-fatal error messages are printed to
     ...
     *               <code>m &gt; 0</code>.
     * @param x      On initial entry this must be set by the user to the values of the
     ...
     * @param diagco Set this to <code>true</code> if the user wishes to provide the
     ...
     *               </ul>
     * @param eps    Determines the accuracy with which the solution is to be found.
     ...
     * @param xtol   An estimate of the machine precision (e.g. 10e-16 on a SUN station
     *               3/60). The line search routine will terminate if the relative
     *               width of the interval of uncertainty is less than
     *               <code>xtol</code>.
     * @param iflag  This must be set to 0 on initial entry to <code>lbfgs</code>. A
     ...
     *               must provide the diagonal matrix <code>Hk0</code>. The following
     *               negative values of <code>iflag</code>, detecting an error, are
     *               possible:
     *               <ul>
    ...
    public static void lbfgs(int n, int m, double[] x, double f, double[] g,
                             boolean diagco, double[] diag, int[] iprint, double eps, double xtol,
                             int[] iflag) throws ExceptionWithIflag {
        ...
        while (true) {
            ...
            double directionMag = ddot(n, w, ispt + point * n, 1, w,
                    ispt + point * n, 1);
            ...
            if (iprint[0] > 0)
                System.err.println("msrch return = nfev=" + nfev[0] + " nfun=" + nfun
                        + " info=" + info[0] + " bestx=" + Mcsrch.stx[0] + " farx="
                        + Mcsrch.sty[0] + " brackt=" + Mcsrch.brackt[0] + " stp=" + stp[0]
                        + " gnorm=" + Math.sqrt(ddot(n, g, 0, 1, g, 0, 1)) + " xnorm="
     ...
     *                plus gradient vector.
     *                </ul>
     * @param iter1   Number of iterations so far.
     * @param nfun1   Number of function evaluations so far.
     * @param gnorm1  Norm of gradient at current solution <code>x</code>.
     ...
     * @param stp11   Current stepsize.
     * @param finish1 Whether this method should print the ``we're done'' message.
     */
     ...
     * from the subroutine <code>daxpy</code> in <code>lbfgs.f</code>. There could
     * well be faster ways to carry out this operation; this code is a straight
     ...
     * <code>ddot</code> in <code>lbfgs.f</code>. There could well be faster ways
     * to carry out this operation; this code is a straight translation from the
     * Fortran.
     */
